...
NOTE: this is currently just a copy of the code I wrote for ffnet....
I'm going to modify it soon for pybrain - and then at some point write a cleaner
version of these functions, making the nn implementation swappable
...
ncols = 72
nrows = 300 # 287420 is the first 9999999
...
def main():
    ...
    x1, x2 = calc_stats(net,sorted_data)
    print('in: ' + str(x1) + ', out:' + str(x2))
    logging.info('in: ' + str(x1) + ', out:' + str(x2))
...
def graph_all_data(data):

    which_cols = [0,13,18,22,28] #just graph these columns
    ...
    for i in xrange(total):
        ...
        '''
        print 'r^2 for: ', which_cols[i]
        r_sqr = scipy.stats.pearsonr(data[:,which_cols[i]], data[:,0])[0]**2
        print r_sqr
        '''
    '''
...
def calc_stats(net, sorted_data):
    ...
    return r_squared_in, r_squared_out
